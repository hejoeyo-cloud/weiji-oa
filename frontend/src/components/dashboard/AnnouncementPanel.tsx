...
export default function AnnouncementPanel({ unreadCount, items, onOpenAll, onMarkRead }: Props) {
  const pinned = items.find(item => item.is_pinned) || items[0]
  const summary = items.find(item => item.id !== pinned?.id) || pinned
    ...
    <section className="rounded-3xl border border-slate-200 bg-white shadow-sm">
      ...
      {!pinned ? (
        ...
        <div className="space-y-3 px-4 py-3">
          ...
          {summary && summary.id !== pinned.id && (
            <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50/70 px-3 py-2">
              <div className={`h-2 w-2 rounded-full ${summary.is_read ? 'bg-slate-300' : 'bg-rose-500'}`} />
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-medium text-slate-800">{summary.title}</p>
                <p className="mt-0.5 text-[11px] text-slate-400">{formatTime(summary.created_at)}</p>
              </div>
              {!summary.is_read && (
                <button
                  onClick={() => onMarkRead(summary.id)}
                  className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-white px-1.5 py-0.5 text-[10px] font-medium text-slate-500 transition-colors hover:border-slate-300 hover:text-slate-900"
                >
                  <Check className="h-2.5 w-2.5" />
                  已读
                </button>
