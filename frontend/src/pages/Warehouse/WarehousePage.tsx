    ...
    <div className="space-y-4">
      ...
      {stats.low_stock_count > 0 && (
        <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: '#f0f0f0' }}>
          <div className="flex items-center gap-2 px-4 py-2.5 border-b" style={{ borderColor: '#f0f0f0', background: '#fef2f2' }}>
            <AlertTriangle className="w-5 h-5" style={{ color: '#dc2626' }} />
            <span className="text-base font-bold" style={{ color: '#dc2626' }}>
              低库存预警（{stats.low_stock_count} 种货品库存 ≤ 20）
            </span>
          </div>
          <div className="p-4 grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 gap-4">
            {[...stats.low_stock_items]
              .sort((a, b) => a.current_qty - b.current_qty)
              .map(item => (
                <div
                  key={item.id}
                  className="rounded-lg p-2.5 border transition-all hover:scale-105"
                  style={{
                ...
                >
                  <div className="text-sm font-semibold mb-1 truncate" style={{ color: '#1f2937' }}>
                    {item.name}
                  </div>
                  <div className="flex items-baseline gap-1">
                    ...
                    <span className="text-xs" style={{ color: '#6b7280' }}>{item.unit}</span>
                  </div>
                </div>
              ))}
          </div>
