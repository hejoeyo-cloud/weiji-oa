/**
 * MessagesPage — Gmail-style inbox powered by react-mail-inbox
 */
import { useEffect, useState, useCallback } from 'react'
import GmailInbox from 'react-mail-inbox'
import 'react-mail-inbox/dist/index.css'
import { Edit2, Star, Trash2, Search, Reply, Forward, Download, Inbox, Send, FileText, Archive, RefreshCw } from 'lucide-react'
...
export default function MessagesPage() {
  ...
  const loadUsers = async () => { try { const u = (await getUsers()).data || (await getUsers()) || []; setUsers(u.map((x:any)=>({...x,email:x.email||x.name+'@oa.local'}))) } catch {} }
  ...
  const handleSend = async (emailData: any) => {
    ...
    try {
      if (mode==='compose') {
        await sendMessage({ to_ids:toIds, subject:emailData.subject, body:emailData.body, draft:false })
        notify('邮件已发送', emailData.subject||'(无主题)')
      } else if (mode==='reply'&&selected) {
        await replyMessage(selected.id, { body:emailData.body, to_ids:toIds })
        notify('回复已发送', 'Re: '+selected.subject)
      } else if (mode==='forward'&&selected) {
        await forwardMessage(selected.id, { body:emailData.body, to_ids:toIds })
        notify('转发已发送', 'Fwd: '+selected.subject)
      }
      setMode('view'); setSelected(null); loadFolder('sent')
  ...
  const fetchEmailOptions = async (q: string): Promise<{name:string, email:string}[]> => {
    if (!q) return users.slice(0,20).map(u=>({name:u.name, email:u.email, id:u.id}))
    const lower = q.toLowerCase()
    return users.filter(u=>u.name.toLowerCase().includes(lower)||(u.email||'').toLowerCase().includes(lower))
      .slice(0,20).map(u=>({name:u.name, email:u.email, id:u.id}))
  }

  const folders: Folder[] = ['inbox','sent','drafts','trash']
  const filtered = messages.filter(m=>!search||(m.subject||'').includes(search)||(m.sender_name||'').includes(search))
    ...
    <div className="flex h-[calc(100vh-80px)] bg-white border rounded-lg overflow-hidden" style={{borderColor:'#e5e7eb'}}>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {mode==='view' && selected ? (
          ...
          /* Compose / Reply / Forward using react-mail-inbox */
          <div className="flex-1 flex flex-col">
            <div className="px-4 py-2.5 border-b flex items-center gap-3" style={{borderColor:'#e5e7eb'}}>
              <button onClick={()=>{setMode('view');setSelected(null)}} className="p-1 hover:bg-gray-100 rounded text-gray-500"><ArrowIcon/></button>
              <span className="font-medium text-sm">{mode==='reply'?'回复':mode==='forward'?'转发':'写邮件'}</span>
            </div>
            <div className="flex-1 overflow-hidden">
              <GmailInbox
                fetchEmailOptions={(q:string)=>fetchEmailOptions(q)}
                handleChange={handleSend}
                initialData={mode==='reply'&&selected?{to:[{name:selected.sender_name||'',email:''}],subject:selected.subject||'',body:''}:mode==='forward'&&selected?{subject:selected.subject||'',body:selected.body||''}:{}}
                initialTheme="light"
                isReply={mode==='reply'}
                showOnlyBody={mode==='forward'}
                minHeight="100%"
                placeholder="撰写邮件内容..."
              />
            </div>
          ...
          <div className="flex-1 flex items-center justify-center text-gray-400">
            <div className="text-center">
              ...
              <div className="text-sm">选择一封邮件查看</div>
            </div>
          </div>
        )}
      </div>
...
function ArrowIcon() { return <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8l5 5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg> }
