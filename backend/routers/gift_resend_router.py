...
def list_records(
    ...
    if search:
        ...
        query = query.filter(
            ...
            | (GiftResendRecord.tracking_no.like(pattern))
        )
    if start_date:
    ...
    # 计算重复订单号
    all_order_nos = [r[0] for r in query.with_entities(GiftResendRecord.order_no).filter(GiftResendRecord.order_no != "").all()]
